	...
	<div id="wrap">
		...
		<div id="content">
								...
								<c:forEach var="person" items="${people}">
									<tr>
										<td><input class="formInput form-control" type="text"
											name="personName1" value="${person.name}" disabled /></td>
										<td><input class="formInput form-control" type="number"
											name="personAge1" value="${person.age}" disabled /></td>
										<td><input class="formInput form-control" type="text"
											name="personAddress1" value="${person.address}" disabled /></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
			...
			<form id="form1" role="form" action="" method="post">
				<h3 id="success">Person successfully added!</h3>
				<h3 id="fail">Person adding failed</h3>
				<div class="form-group">
					<h2>
						<label>Please input the detail information for person </label>
					</h2>
					<h3>
					...
					<h3>
						<input class="form-control" id="personAge" name="personAge"
							placeholder="Age" type="number">
					</h3>
					<h3>
						<input class="form-control" id="personAddress"
							name="personAddress" placeholder="Address" type="text">
					</h3>
		...
		<script>
			...
			$("#btn_submit")
					.click(
							function() {
								var personName = $("input[name='personName']")
										.val();
								var personAge = $("input[name='personAge']")
										.val();
								var personAddress = $(
								...
								if (!($.isNumeric(personAge) && Math
										.floor(personAge) == personAge)) {
													...
													if (json.flag == 1) {
														...
														var tr = $(
																...
																.children(
																		"tbody")
																.append(
																		"<tr><td><input class='formInput form-control' type='text' value="+json.personName+" disabled /> </td><td><input class='formInput form-control' type='text' value="+json.personAge+" disabled /> </td><td><input class='formInput form-control' type='text' value="+json.personAddress+" disabled /> </td>");
													}
													if (json.flag == 2) {
														$("#success").hide();
														$("#fail").show();
