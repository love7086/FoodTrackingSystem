	...
	<div id="wrap">
								...
								<c:forEach var="network" items="${networks}">
									<tr>
										<td><input class="form-control pull-left "
											style="width: 80%" name="networkName" value="${network.name}" />
											<p class="pull-left">&nbsp;&nbsp;&nbsp;</p>
											<button class="btnSave btn btn-info btn-default pull-left">
												Save</button>
											<p class="pull-left">&nbsp;&nbsp;&nbsp;</p>
											<button class="btnDelete btn btn-info btn-default pull-left">
												Delete</button> <input type="hidden" name="networkId"
											value="${network.networkId}" /></td>
		...
		<script>
			$(".btnSave").click(function() {
				var network = $(this).prev().prev().val();

				var networkId = $(this).next().next().next().val();
				$.post("saveNetwork.htm", {
					"network" : network,
					"networkId" : networkId
				});
			});
			$(".btnDelete").click(function() {
				var network = $(this).prev().prev().prev().prev();
				var networkName = network.val();
				$.post("deleteNetwork.htm", {
					"networkName" : networkName
				}, function() {
					network.parent().remove();
				});
			});
		</script>
	</div>
