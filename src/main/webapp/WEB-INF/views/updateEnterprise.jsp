...
<body class="padTop53 ">
	...
	<div id="wrap">
						...
						<table class="table" id="networkTable">
							...
							<tbody>
								<c:forEach var="enterprise" items="${Enterprises}">
									<tr>
										<td><input class="form-control pull-left "
											name="enterpriseName" value="${enterprise.name}" /></td>
										<td><input class="form-control pull-left"
											value="${enterprise.location}" /></td>
										<td><input class="form-control pull-left" disabled
											value="${enterprise.network}" /></td>
										<td>
											<p class="pull-left">&nbsp;&nbsp;&nbsp;</p>
											<button class="btnSave btn btn-info btn-default pull-left">
												Save</button>
											<p class="pull-left">&nbsp;&nbsp;&nbsp;</p>
											<button class="btnDelete btn btn-info btn-default pull-left">
		...
		<script>
			$(".btnSave").click(
					function() {
						var enterpriseId = $(this).next().next().next().val();
						var enterpriseName = $(this).parent().prev().prev()
								.prev().find("input").val();
						var location = $(this).parent().prev().prev().find(
								"input").val();
						$.post("saveEnterprise.htm", {
			...
			$(".btnDelete").click(function() {
				var enterpriseId = $(this).next().val();
				var tr = $(this).parent().parent();
				$.post("deleteEnterprise.htm", {
					"enterpriseId" : enterpriseId
				}, function() {
					tr.remove();
				});
